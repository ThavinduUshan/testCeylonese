  ...
  return new Promise((resolve, reject) => {
    db.getConnection((err, connection) => {
      if (err) {
        ...
        const sql =
          "SELECT partnershipID FROM partnerships WHERE senderProduct = ? OR receiverProduct = ? AND status = ?";
        connection.query(
          sql,
...
const getPartnershipDetails = (partnershipID, res) => {
  return new Promise((resolve, reject) => {
    db.getConnection((err, connection) => {
      if (err) {
        return res.json({ error: "Internal Server Error" });
...
const getPartnershipedProductDetails = (partnershipProductID, res) => {
  return new Promise((resolve, reject) => {
    db.getConnection((err, connection) => {
      if (err) {
        return res.json({ error: "Internal Server Error" });
        ...
        const sql =
          "SELECT sellers.sellerID, stores.storeID, stores.storeName, products.*, product_images.image1, product_images.image2, product_images.image3, product_images.image4,product_images.image5 FROM sellers INNER JOIN stores ON sellers.sellerID = stores.sellerID INNER JOIN products ON products.sellerID = sellers.sellerID INNER JOIN product_images ON products.productID = product_images.productID WHERE products.productID = ?";
        connection.query(sql, [partnershipProductID], (err, results) => {
          if (err) {
            reject();
          } else {
            resolve(results[0]);
...
module.exports = {
  ...
  getPartnershipedProductDetails,
};
